...
def check_portal_main_config_exist(config_path):
    if os.path.exists(config_path):
        ...
        return f"{config_path} does not exist"
...
def z_score_checker(PORTAL_CONFIG_FILE, LOCAL_HTTTP, cohort, protein_name="EGFR"):
    ...
    """
    configs = config.load(PORTAL_CONFIG_FILE)
    report_dir = configs["report_directory"][cohort]
    list_cohorts = list(configs["report_directory"].keys())
