# Retrieved from Picked-Group FDR digest.py
...
def make_protein2peptide_dataframe(fasta_file):
    """
    ...
    protein2peptide_df = pd.DataFrame(
        list(mapping_dic.items()), columns=["protein_identifier", "num"]
    )
    protein2peptide_df["uniprot_id"] = protein2peptide_df["protein_identifier"].apply(
        lambda x: x.split("|")[1]
    )
    protein2peptide_df = protein2peptide_df[
        ~protein2peptide_df.protein_identifier.str.contains("REV__")
    ]
    protein2peptide_df["max_num_peptide"] = protein2peptide_df.groupby("uniprot_id")[
        "num"
    ].transform("max")
