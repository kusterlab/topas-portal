...
def get_all_fusions_per_NGS(x, fusion_dic, pattern_alteration=r"[A-Z1-9]+\|[A-Z1-9,]+"):
    try:
        ...
        return (";").join(
            [fusion_dic.get(x, x) for x in list(set(sum(all_possibilities, start=[])))]
        )
    except:
        return ""


def get_all_cnvs_per_NGS(
    x, gene_name, cnv_dic, pattern_alteration=r"AMP|DEL|GAIN|LOSS"
):
    ...
    try:
        ...
        res = [
            f"{gene_name}_{onkokb_definitions.get(x,x)}"
            for x in list(set(sum(all_possibilities, start=[])))
        ]
...
def get_all_snv_per_NGS(
    X, protein_name, snv_dic, pattern_alteration=r"p.[A-Z][0-9]+[A-Z]"
):
    """
    ...
    try:
        ...
        if column_name == "fusion_onkoKB":
            sub_df[column_name] = sub_df[gene_name].apply(
                lambda x: get_all_fusions_per_NGS(x, onkokb_dic)
            )

        if column_name == "cnv_onkoKB":
            sub_df[column_name] = sub_df[gene_name].apply(
                lambda x: get_all_cnvs_per_NGS(x, gene_name, onkokb_dic)
            )

        if column_name == "snv_onkoKB":
            sub_df[column_name] = sub_df[gene_name].apply(
                lambda x: get_all_snv_per_NGS(x, gene_name, onkokb_dic)
            )

        return df.merge(
            sub_df[["Sample name", column_name]], on="Sample name", how="left"
        )
    except:
        return df
...
def _merge_onkokb_annotation(
    cohorts_db: data_api.CohortDataAPI, abundance_df: pd.dataFrame, identifier: str
):
    df = abundance_df.copy()
    genomics_df = cohorts_db.get_genomics()
    onkokb_dic = cohorts_db.get_oncoKB_annotations()
    try:
        df = add_onkokb_annotation(
            df, genomics_df, identifier, onkokb_dic, column_name="fusion_onkoKB"
        )
    except:
        pass
    ...
    try:
        df = add_onkokb_annotation(
            df, genomics_df, identifier, onkokb_dic, column_name="cnv_onkoKB"
        )
    except:
        pass
    ...
    try:
        df = add_onkokb_annotation(
            df, genomics_df, identifier, onkokb_dic, column_name="snv_onkoKB"
        )
    except:
        pass
...
def get_cnv_from_the_ONKOKB_api(gene_name, cnv_type, oncokb_api_token):
    """
...
def _split_genomics_annotation(x: str):
    try:
        if x == "missing":
            ...
            return {
                "cnv": _clean_annotation(cnv),
                "snv": _clean_annotation(snv),
...
def make_final_genomics_annotation(df):
    try:
        df["snv"] = df.genomics_annotations.apply(
            lambda x: _split_genomics_annotation(x)["snv"]
        )
        df["cnv"] = df.genomics_annotations.apply(
            lambda x: _split_genomics_annotation(x)["cnv"]
        )
        df["fusion"] = df.genomics_annotations.apply(
            lambda x: _split_genomics_annotation(x)["fusion"]
        )
    except:
        pass
    return df
...
def _merge_data_with_genomics_alterations(
    ...
    try:
        ...
        if annotation_type == "genomics_annotations":
            annotated_abundance_df = make_final_genomics_annotation(
                annotated_abundance_df
            )
