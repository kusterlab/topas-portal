  ...
  <v-container class="bg-grey-lighten-3" fluid>
    <v-row>
      <v-col sm="12" md="2" lg="2">
        <v-card variant="flat">
          <v-card-title tag="h1"> PCA/UMAP plots </v-card-title>
          <v-card-text>
        ...
        <v-card variant="flat" class="mt-4">
          <v-card-title tag="h1"> Silhouette Analysis </v-card-title>
          <v-card-text>
            <v-text-field
      ...
      <v-col sm="12" md="10" lg="10">
        ...
        <v-card v-if="silData.length > 0" variant="flat" class="mt-4">
          <v-card-text>
            <v-row>
              <v-col sm="12" md="5" lg="5">
                <silhouette-scores-table v-if="silData.length > 0" :data-source="silData" />
              </v-col>
              <v-col sm="12" md="5" lg="5">
                <lollipop-plot
                  v-if="showPlot"
...
<script>
  ...
  import QcTable from '@/components/tables/QCTable.vue'
  ...
  export default {
    ...
    methods: {
          ...
          if (this.activeMeta.toString() !== 'Sample') {
            ...
            if (this.geneSubsetActive && this.file !== null) {
              ...
              const response = await axios.post(
                `${import.meta.env.VITE_API_HOST}/uploadGenes`,
                formData,
                {
                  headers: {
            ...
            const response = await axios.get(
              `${import.meta.env.VITE_API_HOST}/qc/sil/all/${inputDataType}/${this.cohortIndex}/${dimReductionMethod}/${referenceChannel}/${replicate}/${this.activeMeta}/${numPatient}/${silInputType}/${allOrselected}/${customGroup}/${imputationRatio}`
            )
      ...
      async metaComboUpdater() {
        ...
        const response = await axios.get(`${import.meta.env.VITE_API_HOST}/qc/metadata`)
        const finalIndex = []
      ...
      async getqcData() {
        ...
        try {
          this.isLoading = true
          const response = await axios.get(
            `${import.meta.env.VITE_API_HOST}/qc/${allSelected}/${inputDataType}/${this.cohortIndex}/${dimReductionMethod}/${referenceChannel}/${replicate}/${customGroup}/${imputationRatio}`
          )
      ...
      updateSelectedRows(selectedIds, selectedData) {
        // for customized coloring of the samples if needed
        this.selIds = []
