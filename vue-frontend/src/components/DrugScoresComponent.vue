  ...
  <v-container fluid>
    <v-row class="pa-4 bg-grey-lighten-3">
      <v-col sm="12" md="3" lg="2">
        <v-card variant="flat">
          <v-card-title tag="h1"> Drug Scores </v-card-title>
          <v-card-text>
            ...
            >
              <v-radio label="Compare patients (per drug)" value="per_drug" />
              <v-radio label="Compare drugs (per patient)" value="per_patient" />
            </v-radio-group>
            <v-autocomplete
          ...
          <v-card-text>
            <v-row>
              <v-col sm="12" md="7" lg="7">
                <drugscore-table :data-source="url" @onRowSelect="updateSelectedRows" />
              </v-col>
    ...
    methods: {
      ...
      async getEntitiesList() {
        const response = await axios.get(
          `${import.meta.env.VITE_API_HOST}/patients/${this.cohortIndex}/all_entities`
        )
        const entities = []
      ...
      async getPatientsList() {
        const response = await axios.get(
          `${import.meta.env.VITE_API_HOST}/drugscore/${this.cohortIndex}/patients_list`
        )
        const patients = []
        response.data.forEach(element => {
      ...
      async getDrugsList() {
        const response = await axios.get(
          `${import.meta.env.VITE_API_HOST}/drugscore/${this.cohortIndex}/drugs_list`
        )
        const drugs = []
        response.data.forEach(element => {
      ...
      async getDrugScoresForDrug(key) {
        ...
        const query = `${import.meta.env.VITE_API_HOST}/drugscore/${this.cohortIndex}/drug/${key}/${entities}`
        this.url = query
      ...
      async getDrugScoresForPatient(key) {
        this.drugidentifier = ''
        this.plotData = []
        const query = `${import.meta.env.VITE_API_HOST}/drugscore/${this.cohortIndex}/patient/${key}`
        this.url = query
        const response = await axios.get(query)
        this.swarmShow = true
        this.plotData = response.data
