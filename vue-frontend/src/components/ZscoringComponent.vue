  ...
  <v-container fluid>
    <v-row class="pa-4 bg-grey-lighten-3">
      <v-col sm="12" md="2" lg="2">
        <v-card variant="flat">
          <v-card-title tag="h1"> Subcohort z-scores </v-card-title>
          <v-card-text>
        ...
        <v-card variant="flat" class="mt-4">
          <v-card-title tag="h1"> Select protein/p-peptide </v-card-title>
          <v-card-text>
            <v-select
              v-model="mode"
            ...
            />
            <v-btn :loading="loading" class="mt-4" color="primary" @click="getData">
              Compute z-scores
            </v-btn>
          </v-card-text>
        </v-card>
            ...
            <v-expansion-panels>
              <v-expansion-panel>
                ...
                <v-expansion-panel-text>
                  In this tab you can calculate the z-scores for each group of patients based on the
                  metadata and compare it with the z-scores across all patients.
                </v-expansion-panel-text>
              </v-expansion-panel>
              <v-expansion-panel>
                <v-expansion-panel-title class="mb-0"> How to use </v-expansion-panel-title>
                <v-expansion-panel-text>
                  Select a group of patients based on one of the metadata columns. Then select a
                  protein or phosphopeptide of interest and press the "Compute z-scores" button
                  compute the z-scores within that subcohort.
                </v-expansion-panel-text>
              </v-expansion-panel>
            </v-expansion-panels>
          </v-card-text>
      ...
      <v-col sm="12" md="10" lg="10">
        <v-card v-show="selectionMethod === 'table'" variant="flat" class="mb-4">
          <v-card-text>
            <v-row>
    ...
    methods: {
      ...
      async getData() {
        try {
          ...
          this.loading = true
          this.zscoreTableUrl = `${import.meta.env.VITE_API_HOST}/zscore/${this.mode}/${this.cohortIndex}/${this.identifier}/${this.customGroup}/${this.metadataType}`
