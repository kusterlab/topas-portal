...
<script>
  ...
  export default {
    ...
    props: {
      ...
      qcSelIds: {
        // the index of the selected items from table etc.
        type: Array,
        default: undefined
      },
      qcplotData: {
        // Plot data with pc1,pc2, metadata
        type: Array,
        default: undefined
    ...
    methods: {
      ...
      plotQc(colorShapeAssign) {
        ...
        g.selectAll('.dots')
          ...
          .attr(
            ...
            symbol.type(function (d) {
              return d3[d.shapeCodes.toString()]
            })
          )
          .attr(
            'd',
            symbol.size(function (d) {
              return parseInt(d.rSize)
            })
          )
        ...
        if (this.showLegend) {
          ...
          const legs = svg.selectAll('.dots').data(unique).enter().append('path')
          legs
            .attr(
              'd',
              symbol.type(function (d) {
        ...
        const zoomed = d3
          ...
          .on('zoom', function (event) {
            ...
            g.selectAll('path').attr('transform', d => `translate(${newX(d.pc1)},${newY(d.pc2)})`)
          })
