  ...
  export default {
    ...
    methods: {
      ...
      plotExpression: function () {
        ...
        const xx = d3.scaleBand().range([0, width]).padding(0.9)

        const y = d3.scaleLinear().range([height, minDomain])
          ...
          tooltip
            .html(
              this.completeTooltip
                ? d.label + '_Score: ' + d.value.toFixed(2) + '_' + d.type
                : d.label
            )
            .transition()
            .duration(200) // ms
        ...
        const bars = lollipop.append('g').attr('class', 'bars')
        ...
        if (this.lolliMode) {
          ...
          const circles = lollipop.append('g').attr('class', 'circles')
          ...
          circles
            ...
            .on('end', function () {
              d3.select(this).transition().duration(500).attr('r', lolliradian)
            })
        }
        lollipop
          .append('g')
        ...
        svg.selectAll('circle').data(data).on('mousemove', tipMouseover).on('mouseout', tipMouseout)
        /*
