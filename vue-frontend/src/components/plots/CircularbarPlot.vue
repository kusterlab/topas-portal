<template>
  <div v-if="plotData" :id="plotId" class="circularbarchart">
    <v-btn v-if="savePlot" class="ma-2 float-left" color="primary" @click="downloadSVG">
    ...
    methods: {
      ...
      plotExpression: function () {
        ...
        for (let i = 0; i < data.length; i++) {
          data[i].new_value = data[i].value > 0 ? data[i].value : 0
        }
        ...
        svg
          ...
          .attr('text-anchor', function (d) {
            return (x(d.label) + x.bandwidth() / 2 + Math.PI) % (2 * Math.PI) < Math.PI
              ? 'end'
              : 'start'
          })
