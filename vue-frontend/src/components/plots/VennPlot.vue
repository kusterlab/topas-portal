  ...
  export default {
    ...
    methods: {
      downloadSVG() {
        ...
        if (aPlots.length > 0) {
          utils.downloadSVGs(aPlots, 'overlap Plot', false, 'canvasId', [])
        }
      },
      ...
      plotVennDiagram() {
        ...
        */
        ...
        div.selectAll('g').on('click', function (event, d) {
          ...
          that.uniqProteins = []
          let currentDiff = []
          if (that.showListIntersect) {
            that.selectedProteins = selection._groups[0][0].__data__.proteins
          }
          that.uniqueSize = ''
          if (that.selectedBatch.length === 1) {
            ...
            for (let p = 0; p < uniquegroups.length; p++) {
              if (uniquegroups[p] !== selGrp) {
                currentDiff = memberships.getDifference(
                  currentDiff,
                  memberships.getProteinList(plotSet, uniquegroups[p])
                )
                const uniqueIds = []
            ...
            if (that.showListDiff) {
              that.uniqProteins = currentDiff
            }
          }
