  ...
  export default {
    ...
    methods: {
      ...
      getExpressionInCommonSamples: function (scoreType) {
        ...
        if (this.ensembleData) {
          ...
          const expressionsBySampleId1 = Object.assign(
            {},
            ...this.expressions1.map(x => ({ [x['Sample name']]: x[scoreType] }))
          )
          const expressionsBySampleId2 = Object.assign(
            {},
            ...this.expressions2.map(x => ({ [x['Sample name']]: x[scoreType] }))
          )
      ...
      */
        ...
        const y = d3.scaleLinear().domain([minValueY, maxValueY]).range([height, 0])
        ...
        svg
          ...
          .text(
            this.identifier1 +
              ' ' +
              this.labelX.replace('Intensity', 'log10 Expression') +
              '  (' +
              this.omicsTypeX +
              ')'
          )
        ...
        svg
          ...
          .text(
            this.identifier2 +
              ' ' +
              this.labelY.replace('Intensity', 'log10 Expression') +
              '  (' +
              this.omicsTypeY +
              ')'
          )
