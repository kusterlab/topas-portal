    ...
    methods: {
      ...
      drawPlot: function (data) {
        ...
        this.minValue = Math.min(
          this.xTransformation(this.minDose),
          this.xTransformation(sortedData.at(0))
        )
        this.maxValue = Math.max(
          this.xTransformation(this.maxDose),
          this.xTransformation(sortedData.at(-1))
        )
        ...
        if (this.plotKDE) {
          ...
          const density = this.kde(this.epanechnikov(), thresholds, data, bandwidth)
          // Scale the range of the data in the y domain
        ...
        svg
          ...
          .attr(
            'transform',
            'translate(' + width / 2 + ' ,' + (height + this.margin.top + 50) + ')'
          )
      ...
      drawLines: function (oLines) {
        ...
        lines
          ...
          .append('title')
          .text(d => `Num = ${d.value.toPrecision(3)}`)
        ...
        lines
          ...
          .attr('transform', d => {
            return (
              'translate(' +
              x(Math.min(Math.max(d.value, this.minValue), this.maxValue)) +
              ',' +
              height * 0.25 +
              ')'
            )
