      ...
      drawSwarm(mode) {
        ...
        d3.selectAll('.circ')
          .on('mousemove', function (d) {
            tooltip
              .html(
                `<strong>${title}: ${d.target.__data__[title]}</strong><br>${fieldY}: ${d.target.__data__[fieldY].toFixed(2)}    ${d.target.__data__[fieldX]}</br>`
              )
              .style('top', 200 + 'px')
              .style('left', 200 + 'px')
              .style('opacity', 0.9)
        ...
        if (this.showLegends) {
          ...
          data.filter(element => {
            ...
            if (!isDuplicate) {
              ...
              svg
                .append('circle')
                .attr('cx', width - width / 6)
                .attr('cy', y)
                .attr('r', 6)
                .style('fill', currColor)
              svg
                .append('text')
                .attr('x', width - width / 6.5)
                .attr('y', y)
                .text(item)
