...
const utils = {
  ...
  downloadPNG: function (oCanvas, sSourceSVG, sName) {
    ...
    image.onload = function () {
      ...
      oLink.href = oCanvas.toDataURL('image/svg')
  ...
  downloadSVGs: async function (aSVGs, sName, bDownloadAsSvg, sCanvasId, cssFiles) {
    ...
    newGTextTitle.setAttribute(
      'transform',
      `translate(${parseInt(oCombinedSvg.getAttribute('width'), 10) / 2}, 20)`
    )
  ...
  downloadCSS: async function (path) {
